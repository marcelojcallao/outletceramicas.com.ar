...
trait SheetMetalCuttingsTrait {
    ...
    public function unionSheetMetalCuttingByMts($pr)
    {
        ...
        if ($pr->isCHP) {
            ...
            $sheet_metal_cuttings = $pr->stocks->reduce( function ($acc, $el){
                ...
                if (($index) || $index === 0) {
                    ...
                    if ((int)$el->quantity > 0) {
                        array_push($acc, ['mts' => $el->mts, 'quantity' => $el->quantity, 'product_id' => $el->product_id]);
                    }
                }
            ...
            array_push($sheet_metal_cuttings, ['mts' => $pr->mts_by_unity, 'quantity' => $pr->stock, 'product_id' => $pr->id]);
