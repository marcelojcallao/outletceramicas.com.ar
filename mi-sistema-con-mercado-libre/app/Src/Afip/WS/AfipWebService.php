...
abstract class AfipWebService
{
	...
	public function __construct($service, $environment)
	{
		...
		if ($environment === 'testing' || $environment === 'TESTING') {
			...
			if ($service === 'ws_sr_padron_a13' || $service === 'ws_sr_constancia_inscripcion') {
				$this->cuitRepresentada = 20227339730;
			} else {
				$this->cuitRepresentada = env('WSBASE_CUIT_REPRESENTADA');
			}
		}
		//dd('cuit ' . $this->cuit, 'representada ' . $this->cuitRepresentada, 'environment ' . $environment, ' service ' . $this->service);
