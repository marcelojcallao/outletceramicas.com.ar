...
trait WSAATrait
{
    ...
    public  function sign_TRA($environment)
    {
        $STATUS = openssl_pkcs7_sign(
            $this->TRA,
            $this->TRA_TMP,
            file_get_contents(($environment == 'PRODUCTION' ? $this->CertificadoProduction : $this->CertificadoTesting) ),
            [
                file_get_contents(($environment == 'PRODUCTION' ? $this->PrivadaKeyProduction : $this->PrivadaKeyTesting)), ''
            ],
            [],
            !PKCS7_DETACHED
        );
    ...
    {
        $path= getcwd();
        $client = new \SoapClient(($environment == 'PRODUCTION' 
            ? "file://".WSCONST::WSAA_PRODUCTION 
            : "file://".WSCONST::WSAA_TESTING), 
