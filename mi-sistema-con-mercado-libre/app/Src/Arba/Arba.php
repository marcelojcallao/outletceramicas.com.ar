...
class Arba
{
	...
	public function getAlicuota($arba_response)
	{
		...
		if(array_key_exists('cantidadContribuyentes', $response))
		{
		...
		if(array_key_exists('contribuyentes', $response))
		{
			$contribuyente = (array)$response['contribuyentes']->contribuyente;

			return $contribuyente['alicuotaPercepcion'];
		}
	...
	public function checkErrors($xml)
	{	
		$result = (array) $xml;
		Log::info('///////////////////');
		Log::info(collect($result)->toJson());
		Log::info('///////////////////');
		Log::info('');
		collect($result)->keys()->map(function($i) use ($result) {
			...
			if( substr($i, -5)=== 'Error' ){
				...
				if ($result['mensajeError'] == '<![CDATA[La CUIT ingresada no se encuentra en ningun padron]]/>') {
					
				}else{

					throw new \Exception($result['mensajeError'], $result['codigoError']);
				}
