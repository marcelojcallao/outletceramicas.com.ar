...
class PedidoClienteController extends Controller
{
				...
				if (!is_null($p['product']['id'])) {
					...
					$pc->items()->create(
						[
							...
							'real_mts' => ($p['metros_cuadrados']) ? $p['metros_cuadrados'] * $p['quantity'] : 0,
	...
	public function update_items_of_pedido()
	{
		try {
			...
			$pedido_cliente_id = $pedido['pedido_id'];

			$products = json_decode($pedido['products']);

			collect($products)->map(function ($product) use ($pedido_cliente_id, $pedido) {
				$p = (array) $product;
				$item = PedidoClienteItem::where('pedido_cliente_id', $pedido_cliente_id)
				...
				if ($p['stock'] == 0) {
					...
					$item->neto_import = $item->unit_price * $p['stock'];
					$item->iva_import = $item->neto_import * $item->iva_percentage / 100;
					$item->total = $item->neto_import + $item->iva_import;

					$item->save();
				}

				pedido_cliente_history($pedido, 'se actualiza produto', auth()->user()->id, $item->toArray());
			});

			$cotiz = PedidoCliente::find($pedido_cliente_id);

			//$cotizacion = fractal($cotiz, new PedidoClienteListTransformer())->toArray()['data'];
			$cotizacion = fractal($cotiz, new PedidoClienteDetailShowTransformer())->toArray()['data'];
