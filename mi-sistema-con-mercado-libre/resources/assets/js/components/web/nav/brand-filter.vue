<template>
    <li >
        <a v-tooltip.top-left="msg" @click.prevent="deleteBrandFilter" href="#">{{filter.name}}</a>
    </li>
</template>
...
export default {
		...
		methods : {
				...
				deleteBrandFilter(){
					...
					this.$store.commit('DELETE_BRAND_FILTER', this.filter);
					
                    this.$store.commit('SHOW_BRAND', this.filter.id);
