...
export default {
    ...
    methods: {
        Increment(){
            if (this.data.total_stock > this.data.available_quantity_here) {
                this.quantity++  
            
                this.data.available_quantity_here = this.quantity;
                busVue.$emit('increment_quantity', this.data);
            }
        },

        Decrement(){
            if (this.data.available_quantity_here <= this.data.total_stock) {
                this.quantity--;  
