            ...
            <template slot="id" slot-scope="props">
                <button
                    ...
                    :class="{
                        ...
                        'btn-outline-danger': props.row.id == row_id || props.row.is_editing === 1,
                        'btn-outline-primary': ! (props.row.id == row_id) || ! (props.row.is_editing === 1),
                        }"
                    type="button"
                    :disabled="(( props.row.is_editing === 1 ) && ( parseInt( props.row.is_editing_by_user ) != parseInt( User.id ) ) ) || ( isUpdatingByOtherUser && ( props.row.id == row_id ))"
                    @click="childRowOpen(props.row.id)"
                >
                  <span
                    v-tooltip="props.row.id == row_id ? 'Cerrar pedido' : props.row.is_editing === 1 ? 'Éste pedido ya esta siendo procesado' :'Ver pedido'"
                    class="icon"
                    :class="props.row.id == row_id || props.row.is_editing === 1 ? 'icon-remove': 'icon-plus'"
                  ></span>
                </button>
            </template>
            </v-client-table>
    ...
    export default {
        ...
        methods : {
            ...
            async childRowOpen(id){
                ...
                const response = await axios.post('/api/pedido_cliente/isEditing', {
                    pedido_id: id,
                ...
                if (response) {
                    ...
                    this.$refs.pedido_cliente_list.tableData[index].is_editing = !is_editing;
                }
            ...
            pedidoCreatedListener(){
                ...
                window.Echo.private('pedidoClienteChannel')
                    ...
                    .listen('PedidoClienteBlockedWhenIsOpen', (pedido) => {
                        console.log("🚀 ~ file: PedidoListTable.vue:701 ~ .listen ~ PedidoClienteBlockedWhenIsOpen:", pedido.pedido.code)
                    ...
                    .listen('PedidoClienteUnLockWhenIsClosed', async (pedido) => {
                        console.log("🚀 ~ file: PedidoListTable.vue:711 ~ .listen ~ PedidoClienteUnLockWhenIsClosed:", pedido.pedido.code)
