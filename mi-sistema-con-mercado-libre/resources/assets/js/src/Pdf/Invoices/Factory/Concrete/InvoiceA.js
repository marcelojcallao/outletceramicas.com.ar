...
class InvoiceA extends InvoiceTypeA {
    ...
    details_of_product(title){
        ...
        collect(this.voucher.details_product).each((product, index) => {
            ...
            console.log("🚀 ~ file: InvoiceA.js:55 ~ InvoiceA ~ collect ~ product:", product)
            if (product.isCHP) {
            ...
            if (product_name_width.w > 145) {
                let array_text = this.pdf.splitTextToSize(description, 145);
            ...
            this.pdf.text(this.CurrencyFormat(product.iva_import / product.quantity) , width_position, height_position, options);
        ...
        if (this.voucher.totals) {
            ...
            collect(this.voucher.totals).each((total, index) => {
                if (typeof total === "object") {
                    ...
                    this.pdf.text(total.iva_name, this.first_column_text() * 8.5, height_position, options);
                    ...
                    this.pdf.text(this.CurrencyFormat(total.iva_import), this.first_column_text() * 11.5, height_position, options);
